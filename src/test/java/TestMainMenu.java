...
public class TestMainMenu extends Settings {
    ...
    public void testCategory() throws InterruptedException {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            int menuItems =
                    driver.findElements(By.xpath("//div[@id = 'gmenu-tab-327']/nav[@class='header__nav-list" +
                            "']/div" + "/a")).size() - 4;
            Random random = new Random();
            int randomItem = random.nextInt(menuItems) + 1;
            ...
            int menuItemsInner =
                    driver.findElements(By.xpath("//div[@id='gmenu-tab-327']//div[@class='header__nav" + "-list" +
                            "-item'][" + randomItem + "]//div[@class='header__drop-inner']/div[@class='header__drop" + "-category-col" + "']//a")).size();
            ...
            int catalogItems =
                    driver.findElements(By.xpath("//a[@class='catalog__item catalog__item--new " + "owox" + "-catalog" +
                            "-list']")).size();
            ...
            cartItemPrice.add(driver.findElement(By.xpath("//div[@id='offer-price']/div")).getText().replaceAll(" ",
                    "").replaceAll("₽", ""));
        ...
        for (int i = 0; i < 2; i++) {
            String priceOfProductInCart =
                    driver.findElement(By.xpath("//div[@class='basket__item-price']/div")).getText().replaceAll(" ",
                            "").replaceAll("₽", "");
            Assert.assertTrue(cartItemPrice.contains(priceOfProductInCart));
            String nameOfProductInCart =
                    driver.findElement(By.xpath("//div[@class='basket__item-title']")).getText().toLowerCase().replaceAll("[^\\da-zA-Z]", "");
            ...
            String sizeOfProductInCart =
                    driver.findElement(By.xpath("//div[@class='basket__item-prop']")).getText().replaceAll(" ", "").toLowerCase().replaceAll("размер:", "");
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class, " + "'basket__empty" +
                "-title')]")));
